...
class PongGame(AsyncWebsocketConsumer):
	async def connect(self):
		...
		await self.accept()  # Accept the connection from the client
	...
	async def send_stream_data(self):
		...
		while self.streaming:
			await self.send(text_data="Streamed data from server")  # Send data to client
			await asyncio.sleep(1)  # Wait 1 second before sending more data
